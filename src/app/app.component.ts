...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.input2.setValidators([ReactiveFormsValidators.requiredIfMatchValidator(this.input1, '1')]);

    // Listeners
    this.form.get('input2')?.statusChanges.pipe(
      tap(() => {
        this.input2IsRequired = this.formHelper.isRequired(this.input2 as FormControl);
      })
    ).subscribe();
  }
}
